...
// Custom hooks
const useDialogForm = (open, initialData) => {
    ...
    const canSave = useMemo(() => {
        return name.trim().length > VALIDATION_RULES.minNameLength && 
               instructions.trim().length > VALIDATION_RULES.minInstructionsLength && 
               !saving;
...
const useKeyboardShortcuts = (open, setOpen, handleSave) => {
    useEffect(() => {
        if (!open) return;
        const onKey = (e) => {
            if (e.key === "Escape") {
...
const NameInput = ({ name, setName, inputRef }) => (
...
const DialogFooter = ({ onClose, onSave, canSave, saving, isEdit }) => (
...
const NewAgentDialog = ({ open, setOpen, onSave, initialData = null }) => {
    const {
        ...
        canSave,
        reset,
    } = useDialogForm(open, initialData);
    ...
    useKeyboardShortcuts(open, setOpen, handleSave);
