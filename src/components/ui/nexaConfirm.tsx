...
/**
 * Small client-side hook to detect mobile (adjust breakpoint if you want).
 * We call it inside the Dialog component as `const isMobile = useIsMobile();`
 */
...
export function nexaConfirm({
    ...
    return new Promise((resolve) => {
        ...
        function Dialog() {
            // close on ESC (keeps behavior identical)
            React.useEffect(() => {
            ...
            return (
                ...
                >
                    ...
                    {/* Mobile drawer - same design but slides from bottom */}
                    {isMobile && (
                        <div className="absolute inset-x-0 bottom-0 px-4 pb-6">
                            <div
                                className="relative mx-auto w-full max-w-none rounded-t-xl overflow-hidden border border-white/5"
                                style={{
                                    background:
                                        "linear-gradient(180deg, rgba(7,11,19,0.95) 0%, rgba(10,14,22,0.88) 100%)",
                                    boxShadow:
                                        "0 20px 60px rgba(2,6,23,0.6), 0 0 40px rgba(244,63,94,0.06)",
                                    animation: "nexaConfirmDrawerIn 220ms cubic-bezier(.22,1,.36,1)",
                                }}
                            >
                                {/* Accent bar (keeps the same gradient look) */}
                                <div className="absolute top-0 left-0 right-0 h-1">
                                    <div
                                        className="h-full w-full"
                                        style={{ background: "linear-gradient(90deg,#F43F5E,#6366F1)" }}
                                ...
                                {/* Close (still top-right inside drawer) */}
                                <button
                                    onClick={() => close(false)}
                                    aria-label="Close"
                                    className="absolute right-3 top-3 z-10 rounded-md p-1.5 text-gray-300 hover:text-white hover:bg-white/10 transition-colors"
                    ...
                    {/* animations (desktop + drawer variants) */}
                    <style>{`
